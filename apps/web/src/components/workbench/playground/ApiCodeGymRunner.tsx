/**
 * @file apps/web/src/components/workbench/playground/ApiCodeGymRunner.tsx
 * @description 3-Star Code Gym muscle memory engine for Station 04: API Forge
 * Trace -> Cloze -> Sprint -> Consumer for Tasks 1..6 (C# & Go)
 */
...
export const ApiCodeGymRunner: React.FC = () => {
  ...
  const {
    ...
    useShallow((s) => ({
      apiState: s.apiState,
      applyApiExecution: s.applyApiExecution,
  ...
  const currentTaskIndex = API_FORGE_TASKS.findIndex((t) => t.id === currentTask.id);
  const nextTask =
    currentTaskIndex >= 0 && currentTaskIndex < API_FORGE_TASKS.length - 1
      ? API_FORGE_TASKS[currentTaskIndex + 1]
      : null;

  // ── Execution Helper ──
  const runApiExecution = useCallback(
    async (code: string) => {
      const res = executeApiScript(code, apiState);
      if (res.success) {
        applyApiExecution(res.newState);
      }
      return res;
    },
    [apiState, applyApiExecution]
  );
  ...
  const fileName = useMemo(
    () => (codeLang === "go" ? "main.go" : "Program.cs"),
    [codeLang]
  );

  const allApiTasksCompleted = useMemo(() => {
    return API_FORGE_TASKS.every((task) => (taskMasteryStars[task.id] || 0) >= 1);
  }, [taskMasteryStars]);
  ...
  const handleVerifyCloze = useCallback(async () => {
    ...
    const validation = currentTask.validate(apiState, res.newState, res, typedCode);
  ...
  const handleVerify = useCallback(() => {
    if (activeRound === 1) {
      // Trace mode complete handled in onRoundComplete or verify
      handleVerifyCloze();
  ...
  }, [activeRound, isTimerRunning, handleVerifyCloze, handleStartSprint, handleRunSprint, handleRunTransfer]);
      ...
      <div className="p-4 rounded-2xl bg-[#EFEAE1] border border-paper-border shadow-paper-sm space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`font-mono text-[10px] font-bold uppercase ${
                      isCurrent ? "text-cyan-400" : "text-ink-muted"
                    }`}
                  >
                    #{idx + 1} API
                  </span>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md bg-cyan-500/20 text-cyan-600 border border-cyan-500/30">
                  API Forge Gym
                </span>
          ...
          <div className="flex items-center gap-2">
            {allApiTasksCompleted && (
              <button
                onClick={() => {
                  audioFx.playSuccessFanfare();
                  setApiVictoryModalOpen(true);
                }}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-stone-900 font-display font-extrabold text-xs shadow-sm transition-all cursor-pointer hover:scale-105 active:scale-95"
                title={t("apiForge.certTooltip", "Отримати сертифікат API Architect")}
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  {starsEarned >= round && (
                    <span
                      className={
                        round === 4
                          ? "text-cyan-400 text-xs drop-shadow-[0_0_6px_rgba(6,182,212,0.8)]"
